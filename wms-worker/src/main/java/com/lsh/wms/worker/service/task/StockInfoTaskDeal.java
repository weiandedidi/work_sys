 ...
 */
@Component
public class StockInfoTaskDeal implements IScheduleTaskDealSingle<String> {
    @Reference
    private IStockQuantRpcService iStockQuantRpcService;

    private static Logger logger = LoggerFactory.getLogger(StockInfoTaskDeal.class);

    public boolean execute(String task, String ownSign) throws Exception {
        try {
            iStockQuantRpcService.updateStockInfoList();
            iStockQuantRpcService.updateStockLocationInfoList();
        }catch (Exception e){
