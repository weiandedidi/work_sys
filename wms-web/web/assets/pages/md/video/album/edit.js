define(function (require, exports, module) {
    var StarChoose = require("../../star/choose");
    ...
    var PageEdit = function () {
        ...
        this.starChoose = new StarChoose();
    }
    ...
    PageEdit.prototype.loadContent = function () {
        var that = this;
        var url = App.ctx + "/md/video/album/page/view" + " " + this.el_form;
    ...
    PageEdit.prototype.deleteImg = function (imageType) {
        ...
        if($("#id").val()){
            //数据库删除
            $.ajax({
                url: App.ctx + "/md/video/album/deleteImage",
                type: "post",
    ...
    PageEdit.prototype.initValidate = function () {
        ...
        this.validate = $(this.el_form).validate({
            ...
            rules: {
                ...
                chn: {required: true, minlength: 1}
            },
            messages: {
                title: {required: "标题必填！"},
                busType: {required: "业务大类不能为空！"},
                duration: {required: "视频时长必填！"},
                chn: {required: "请至少选择一个频道！"}
            },
    ...
    PageEdit.prototype.btnSave = function () {
        ...
        //获取频道
        var chns = "";
        $("#form_page").find("input[name=chn]").each(function(i){
            if(this.checked){
                chns += $(this).attr("id") + "@";
            }
        })
        ...
        // 保存请求
        $.ajax({
            url: App.ctx + "/md/video/album/save",
            type: "post",
            data: data,
